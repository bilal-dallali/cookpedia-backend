...
app.get("/get-comments-from-recipe-id-order-asc/:recipeId", (req, res) => {
    ...
    db.query("SELECT comments.*, users.full_name AS fullName, users.profile_picture_url AS profilePictureUrl FROM comments JOIN users ON comments.user_id = users.id WHERE comments.recipe_id = ? ORDER BY comments.created_at ASC", [recipeId], (err, results) => {
        if (err) {
            console.error("Database error:", err);
